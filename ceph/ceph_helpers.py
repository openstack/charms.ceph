...
def validator(value, valid_type, valid_range=None):
    ...
    :return:
    """
    assert isinstance(value, valid_type), "{} is not a {}".format(value,
                                                                  valid_type)
    if valid_range is not None:
        ...
        # Integer, float should have a min and max
            ...
            if len(valid_range) != 2:
                raise ValueError("Invalid valid_range list of {} for {}.  "
                                 "List must be [min,max]".format(valid_range,
                                                                 value))
...
class PoolCreationError(Exception):
    """
    A custom error to inform the caller that a pool creation failed.  Provides
    an error message
    """
...
class Pool(object):
    ...
    def add_cache_tier(self, cache_pool, mode):
        ...
        check_call(['ceph', '--id', self.service, 'osd', 'tier', 'add',
                    self.name, cache_pool])
        check_call(['ceph', '--id', self.service, 'osd', 'tier', 'cache-mode',
                    cache_pool, mode])
        check_call(['ceph', '--id', self.service, 'osd', 'tier', 'set-overlay',
                    self.name, cache_pool])
        check_call(['ceph', '--id', self.service, 'osd', 'pool', 'set',
                    cache_pool, 'hit_set_type', 'bloom'])
    ...
    def remove_cache_tier(self, cache_pool):
        ...
        :param cache_pool: six.string_types.  The cache tier pool name to
        remove.
        :return: None
        """
        ...
        if mode == 'readonly':
            check_call(['ceph', '--id', self.service, 'osd', 'tier',
                        'cache-mode', cache_pool, 'none'])
            check_call(['ceph', '--id', self.service, 'osd', 'tier', 'remove',
                        self.name, cache_pool])
            ...
            check_call(['rados', '--id', self.service, '-p', cache_pool,
                        'cache-flush-evict-all'])
            check_call(['ceph', '--id', self.service, 'osd', 'tier',
                        'remove-overlay', self.name])
            check_call(['ceph', '--id', self.service, 'osd', 'tier', 'remove',
                        self.name, cache_pool])
...
class ErasurePool(Pool):
    ...
    def create(self):
        if not pool_exists(self.service, self.name):
            ...
            # Create it
            cmd = ['ceph', '--id', self.service, 'osd', 'pool', 'create',
                   self.name, str(pgs), str(pgs), 'erasure',
                   self.erasure_code_profile]
            try:
                check_call(cmd)
            except CalledProcessError:
...
def get_mon_map(service):
    ...
    try:
        ...
        log("mon_status command failed with message: {}".format(e.message))
        raise
    ...
    try:
        ...
        if monitor_list['monmap']['mons']:
            for mon in monitor_list['monmap']['mons']:
                hash_list.append(hashlib.sha224(mon['name'].encode(
                    'utf-8')).hexdigest())
...
def monitor_key_set(service, key, value):
    ...
    """
    try:
        check_output(['ceph', '--id', service, 'config-key', 'put', str(key),
                      str(value)])
    except CalledProcessError as e:
        log("Monitor config-key put failed with message: {}".format(e.output))
        raise
...
def monitor_key_exists(service, key):
    ...
    """
    try:
        check_call(['ceph', '--id', service, 'config-key', 'exists', str(key)])
...
def get_erasure_profile(service, name):
    ...
    try:
        out = check_output(
            ['ceph', '--id', service, 'osd', 'erasure-code-profile', 'get',
             name, '--format=json'])
...
def snapshot_pool(service, pool_name, snapshot_name):
    ...
    :return: None.  Can raise CalledProcessError
    """
    cmd = ['ceph', '--id', service, 'osd', 'pool', 'mksnap', pool_name,
           snapshot_name]
    try:
        check_call(cmd)
    except CalledProcessError:
...
def remove_pool_snapshot(service, pool_name, snapshot_name):
    ...
    :return: None.  Can raise CalledProcessError
    """
    cmd = ['ceph', '--id', service, 'osd', 'pool', 'rmsnap', pool_name,
           snapshot_name]
    try:
        check_call(cmd)
    except CalledProcessError:
...
def remove_erasure_profile(service, profile_name):
    ...
    Updates
    the profile if it exists. Please see http://docs.ceph.com/docs/master/
    rados/operations/erasure-code-profile/
    for more details
    :param service: six.string_types. The Ceph user name to run the command
    under
...
def create_erasure_profile(service,
                           profile_name,
    ...
    :param failure_domain: six.string_types.  One of ['chassis', 'datacenter',
        'host', 'osd', 'pdu', 'pod', 'rack', 'region', 'room', 'root', 'row'])
    ...
    cmd = ['ceph', '--id', service, 'osd', 'erasure-code-profile', 'set',
           profile_name, 'plugin=' + erasure_plugin_name,
           'k=' + str(data_chunks), 'm=' + str(coding_chunks),
           'ruleset_failure_domain=' + failure_domain]
    if locality is not None and durability_estimator is not None:
        raise ValueError(
            "create_erasure_profile should be called with k, m and one of l "
            "or c but not both.")
...
def erasure_profile_exists(service, name):
    ...
    try:
        check_call(['ceph', '--id', service, 'osd', 'erasure-code-profile',
                    'get', name])
        return True
    except CalledProcessError:
        return False
...
def make_filesystem(blk_device, fstype='ext4', timeout=10):
    ...
    while not os.path.exists(blk_device):
        if count >= timeout:
            log('Gave up waiting on block device %s' % blk_device, level=ERROR)
            raise IOError(e_noent, os.strerror(e_noent), blk_device)

        log('Waiting for block device %s to appear' % blk_device, level=DEBUG)
        count += 1
        time.sleep(1)
    else:
...
def ensure_ceph_storage(service,
    ...
    if not filesystem_mounted(mount_point):
        ...
        for svc in system_services:
            log('Starting service {} after migrating data.'.format(svc),
                level=DEBUG)
            service_start(svc)
...
class CephBrokerRq(object):
    ...
    def add_op_create_pool(
            self, name, replica_count=3,
            pg_num=None, weight=None):
        ...
        self.ops.append({'op': 'create-pool',
                         'name': name,
    ...
    @property
    def request(self):
        return json.dumps({'api-version': self.api_version,
                           'ops': self.ops,
...
def get_request_states(request, relation='ceph'):
    ...
    for rid in relation_ids(relation):
        ...
        requests[rid] = {'sent': sent, 'complete': complete, }
